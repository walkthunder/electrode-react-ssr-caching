...
let config = require("./config").defaultConfig;  // eslint-disable-line
let lruCacheConfig = require("./config").defaultLruCacheConfig; // eslint-disable-line
...
exports.clearCache = function () {
  exports.cacheStore = cacheStore = new CacheStore(lruCacheConfig);
};
...
exports.cacheHitReport = function () {
  const hitReport = {};
  cacheStore.cache.keys().forEach((key) => {
...
exports.setCachingConfig = function (cfg) {
  ...
  const userCfg = _.pickBy(_.omit(cfg, ["components", "lruCacheConfig"]), (item) => {
  ...
  const userLruCacheConfig = cfg.lruCacheConfig;
  if (userLruCacheConfig) {
    Object.keys(userLruCacheConfig).forEach((k) => {
      lruCacheConfig[k] = userLruCacheConfig[k];
    });
  }
  _.defaults(lruCacheConfig, cfg.lruCacheConfig);
};
exports.lruCacheConfig = lruCacheConfig;
